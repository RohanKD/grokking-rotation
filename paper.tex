...
To verify the model actually uses the rotation signal rather than ignoring it,
...
target view). Table~\ref{tab:ablation} reports Q1 and Q4 test LPIPS for both
conditions.
...
\midrule
Normal $\Delta\theta$ & \textbf{ABLATION\_Q1\_NORMAL} & \textbf{ABLATION\_Q4\_NORMAL} \\
Shuffled $\Delta\theta$ & ABLATION\_Q1\_SHUFFLED & ABLATION\_Q4\_SHUFFLED \\
Degradation & ABLATION\_DELTA\_Q1 & ABLATION\_DELTA\_Q4 \\
\bottomrule
\end{tabular}
\caption{$\Delta\theta$ ablation at $N=40$. Shuffling the rotation signal
degrades Q4 (complex) substantially while leaving Q1 (simple) relatively
unchanged---confirming that the model exploits $\Delta\theta$ for structural
rotation but not for the copy-source shortcut.}
\label{tab:ablation}
\end{table}

The asymmetric degradation confirms that simple objects are served by a
shortcut that does not require the rotation signal (the source image alone
suffices), whereas complex objects depend on $\Delta\theta$ to generate a
plausible rotated view---directly addressing the shortcut vs.\ structure dichotomy.
...
established in the original algorithmic-task setting. The $\Delta\theta$
ablation (Table~\ref{tab:ablation}) provides a concrete functional signal
complementing the training-curve evidence: for complex objects the model uses
the rotation input; for simple objects it does not need to. Mechanistic probing
of the rotation representation is a natural next step.
