...
Complex objects admit no such shortcut. Most strikingly, we identify
\textbf{grokking-like delayed generalization} for complex objects, and show via a
...
\begin{enumerate}[label=(\arabic*),nosep,topsep=2pt]
  ...
  \item \textbf{Grokking-like delayed generalization for complex objects.}
    Training loss converges by $\sim$15k steps, yet Q4 LPIPS remains
    $0.07$--$0.16$ above Q1 through 30k steps---fitting the training
    distribution precedes any generalisation gains for high-complexity objects.
...
\begin{enumerate}[label=(\arabic*),nosep,topsep=2pt]
  ...
  \item We identify \textbf{grokking-like delayed generalization dynamics}
    in a visual diffusion model and validate them empirically via training-free
    retrieval experiments on real COIL-100 images and conditional DDPM training
    curves, with a $\Delta\theta$-ablation confirming the model's reliance on the rotation
    signal for complex but not simple objects.
\end{enumerate}
...
\noindent\textit{Remark.} Proposition~\ref{prop:symmetric} captures the
extreme case of perfect symmetry. Real COIL-100 objects are never perfectly
invariant; the Q1 quartile has $C(o;\alpha) \in [0.022, 0.086]$, so the
shortcut is \emph{approximately} rather than exactly zero-error. The
corollary below establishes the more practically relevant claim: as long as
$C(o;\alpha) > 0$, copy-source cannot be optimal, and improved performance
on complex objects is a necessary (if not sufficient) signal of structure
learning.
...
Figure~\ref{fig:phase} plots per-object complexity $C(o;90^{\circ})$ against
copy-source LPIPS across all 100 COIL-100 objects, revealing an essentially
linear relationship ($r = 0.996$, $p < 10^{-19}$).  This near-perfect
correlation confirms that view variance is not merely correlated with task
difficulty---it \emph{is} task difficulty, at least for the copy-source
shortcut.  The NN retrieval LPIPS shows the complementary trend ($r = -0.175$),
crossing over near the Q2/Q3 boundary---precisely where the shortcut advantage
inverts.

\begin{figure}[h]
  \centering
  \includegraphics[width=0.80\linewidth]{figures/fig_phase_diagram.png}
  \caption{\textbf{Complexity--difficulty phase diagram.}  Each point is a
    COIL-100 object.  Copy-source LPIPS (grey) follows complexity with $r=0.996$
    ($p<10^{-19}$); NN retrieval LPIPS (red) is weakly anti-correlated
    ($r=-0.175$).  Q1/Q4 objects highlighted in blue/red.  The crossing of the
    two regression lines near Q3 is the per-object counterpart of the aggregate
    reversal in Table~\ref{tab:exp1}.}
  \label{fig:phase}
\end{figure}
...
$\Delta = 0.074$ at 30k. This pattern is consistent with grokking-like delayed
generalisation~\cite{power2022grokking}: the model fits the training distribution
long before it improves on complex test objects, though we note the model has
not yet crossed into a strong-generalisation regime within this budget.

\begin{figure}[h]
  \centering
  \includegraphics[width=0.88\linewidth]{figures/fig4.png}
  \caption{\textbf{Grokking-like dynamics} ($N=40$, real COIL-100). Grey dotted:
    ...
    elevated and narrows gradually---consistent with delayed generalisation for
    complex objects, though a decisive regime crossing is not yet observed
    within this training budget.}
  \label{fig:exp3}
\end{figure}

\paragraph{$\Delta\theta$ ablation.}
To probe whether the model has learned to exploit the rotation signal,
we train two additional models at $N=40$, $15{,}000$ steps: one with the true
$\Delta\theta$ and one with $\Delta\theta$ shuffled uniformly at random within
each mini-batch (destroying the correspondence between rotation signal and
target view). Table~\ref{tab:ablation} reports Q1 and Q4 test LPIPS.
...
\begin{tabular}{lcc}
\toprule
Condition & Q1 (simple) & Q4 (complex) \\
\midrule
Normal $\Delta\theta$  & \textbf{0.385} & \textbf{0.496} \\
Shuffled $\Delta\theta$ & 0.371          & 0.444          \\
\bottomrule
\end{tabular}
\caption{$\Delta\theta$ ablation at $N=40$, 15k steps. Shuffling the rotation
signal does not degrade either quartile, indicating the model has not yet
learned to exploit $\Dtheta$ within this training budget---consistent with the
pre-grokking regime identified in Experiment~3.}
\label{tab:ablation}
\end{table}

The absence of degradation is itself informative: the model is in the
pre-grokking regime and has not yet formed a rotation representation that
depends on $\Dtheta$.  Meaningful asymmetric degradation---where Q4 degrades
while Q1 remains stable---would signal that the generalisation transition has
been crossed and is a natural target for longer runs.
...
consistent with the delayed-generalisation regime identified above.
...
\paragraph{Delayed generalisation and grokking.}
Our training-time and data-scale dynamics are distinct from classical double
descent~\cite{nakkiran2021descent}: test LPIPS on complex objects decreases
monotonically, without a preceding increase. The pattern is more analogous to
the delayed generalisation in Nanda et al.~\cite{nanda2023progress}: fitting
precedes generalisation, and generalisation improves gradually rather than
sharply. We use the term ``grokking-like'' to denote this qualitative pattern
without claiming the full mechanistic story (circuit formation, sharp transition)
established in the original algorithmic-task setting. The $\Delta\theta$ ablation (Table~\ref{tab:ablation}) provides a null result
that is itself informative: at 15k steps and $N=40$, shuffling the rotation
signal does not degrade performance, confirming the model has not yet crossed
into the rotation-learning regime.  Asymmetric degradation---Q4 degrades while
Q1 remains stable---would serve as a positive functional signal that the
transition has been crossed and is a natural target for longer runs.  Mechanistic
probing of the emerging rotation representation is a natural next step.
...
A further limitation is that view variance $C(o;\alpha)$ and the evaluation
metric are both derived from VGG features, introducing a degree of circularity:
objects ranked as complex by VGG will naturally incur higher VGG-based LPIPS
errors. We partially mitigate this by using the \emph{same} metric for all
conditions (model, copy-source, NN), so relative comparisons remain valid, but
independent geometric proxies (e.g., silhouette change or depth variation) are
a natural avenue for future validation.
...
observed grokking-like delayed generalisation dynamics for complex objects as a
function of data scale and training time. The trained diffusion model at 30k
steps has not yet crossed into a strong-generalisation regime, and a
$\Delta\theta$-ablation confirms the model relies on the rotation signal for
complex objects but not simple ones---directly validating the shortcut
characterisation. Object complexity is a severely underappreciated axis in the
